...
> **Deprecated**：本包已移至 `Agent/_legacy/`，不再加入 OpenClaw `extraDirs`。请使用 [`../../burnpal_skill/`](../../burnpal_skill/) 与上游 [burnpal.skill](https://github.com/CCLYX/burnpal.skill)。

小爪（健身减脂本地生活管家）早期 OpenClaw Skill 包。**仅使用 mock 数据，不接真实美团/活动平台 API。**
